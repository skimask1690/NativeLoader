...
void *          BuildIndirectSyscall(SYSCALL_CTX *ctx, DWORD ssn, void *sysaddr);
void            FreeSyscallStub(SYSCALL_CTX *ctx, PVOID stub);

NTDLL_DISK_CTX  MapNtdllFromDisk(HMODULE hNtdll);
DWORD           ResolveSSN(SYSCALL_CTX *ctx);
void *          ResolveSyscall(SYSCALL_CTX *ctx);
...
#define SYSCALL_PREPARE(ctx, fn_hash) \
    do { \
        ctx->export = GetExport(&ctx->ntdll, fn_hash); \
        ssn         = ResolveSSN(ctx); \
        sysaddr     = ResolveSyscall(ctx); \
    } while (0)
...
#define SYSCALL_CLEANUP(ctx) \
    do { \
        NTSTATUS (NTAPI *NtUnmapViewOfSection)(HANDLE, PVOID) = (void *)myGetProcAddressH(ctx->hNtdll, ntunmapviewofsection); \
