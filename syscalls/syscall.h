...
void *          BuildIndirectSyscall(SYSCALL_CTX *ctx, DWORD ssn, void *syscall_addr);
...
#define SYSCALL_PREPARE(ctx, name)                  \
    do {                                            \
        ...
        sysaddr = ResolveSyscall(&ntdll_ctx, name);\
    } while (0)
...
void DestroySyscallContext(SYSCALL_CTX *ctx) {
    ...
    NTSTATUS (NTAPI *NtFreeVirtualMemory)(HANDLE, PVOID*, PSIZE_T, ULONG) =
        (void *)myGetProcAddress(myGetModuleHandleA(ntdll_dll), ntfreevirtualmemory);
...
NTDLL_DISK_CTX MapNtdllFromDisk(void) {
    ...
    for (WORD i = 0; i < nt->FileHeader.NumberOfSections; i++) {
        IMAGE_SECTION_HEADER *sect =
            (IMAGE_SECTION_HEADER *)((BYTE *)nt + sizeof(IMAGE_NT_HEADERS) + i * sizeof(IMAGE_SECTION_HEADER));
        if ((sect->Characteristics & IMAGE_SCN_CNT_CODE) ||
            sect->VirtualAddress <= nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress) {
...
void *BuildIndirectSyscall(SYSCALL_CTX *ctx, DWORD ssn, void *syscall_addr) {
    ...
    SYSCALL_STUB *node = NULL; SIZE_T node_size = sizeof(*node);
    NtAllocateVirtualMemory((HANDLE)-1, (PVOID *)&node, 0, &node_size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);

    PVOID base = NULL; SIZE_T size = 22;
