...
static ULONG_PTR ResolveExport(HMODULE mod,
    const char * name, int isOrdinal) {
    HMODULE curMod = mod;
        ...
        if (rl.Size) {
            ...
            while ((BYTE * ) r < end && r -> SizeOfBlock) {
                ...
                for (DWORD i = 0; i < cnt; i++)
                    if ((list[i] >> 12) == IMAGE_REL_BASED_DIR64) * ((ULONG_PTR * )((BYTE * ) base + r -> VirtualAddress + (list[i] & 0xFFF))) += delta;
